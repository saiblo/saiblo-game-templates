...
在实际游戏开发中，**我们鼓励调整游戏规则，使一个游戏回合中只有一名玩家需要交互**。这有助于减轻游戏开发者的思维难度，也有助于在比赛玩家出现异常时进行妥善的处理。游戏开发者可以通过调用`getState()`直接获取当前的**计时回合**数。当然，框架本身也提供了自行维护游戏回合计数的自由。
...
`messages`为<整数, 字符串>对组成的向量，每一对<整数, 字符串>表示需要发送消息的 AI 编号和消息内容。
